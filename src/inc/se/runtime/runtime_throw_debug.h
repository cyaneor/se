 ...
 * с полной информацией об ошибке и трассировкой, когда включены опции
 * `SE_COMPILE_OPTION_DEBUG` и `SE_LIBRARY_OPTION_ERROR_DESC`.
 *
 ...
 *
 * @note Поле `trace` структуры `se_except_frame_t` заполняется данными `__TIMESTAMP__`,
 *       `__FILE_NAME__` и `__FUNCTION__` для отладки.
 *
...
#        define se_runtime_throw(code, desc, ...)                                                  \
            ...
            {                                                                                      \
                ...
                {                                                                                  \
                    ...
                    prev->trace = (se_except_trace_t){__TIMESTAMP__, __FILE__, __FUNCTION__};      \
                                                                                                   \
 ...
 * @brief Выбрасывает исключение в runtime с кодом ошибки и трассировкой (режим отладки).
 *
 * Этот макрос используется для генерации исключения в runtime окружении с трассировкой,
 * когда включена опция `SE_COMPILE_OPTION_DEBUG`, но отключена `SE_LIBRARY_OPTION_ERROR_DESC`.
 *
 * Он передает управление предыдущему фрейму исключений через `longjmp`,
 * устанавливая код ошибки и данные трассировки (временная метка, имя файла, имя функции).
 *
 * Если текущий фрейм исключений начальный (`se_runtime_except_frame_is_begin() == true`),
 * выполняется возврат указанных значений (`__VA_ARGS__`) без перехода.
 *
 ...
 *
 * @note Поле `trace` структуры `se_except_frame_t` заполняется данными `__TIMESTAMP__`,
 *       `__FILE_NAME__` и `__FUNCTION__` для отладки. Описание ошибки не поддерживается
 *       в этой конфигурации.
 *
 * @warning Использует `longjmp` для перехода,
 *          что требует корректной инициализации фреймов исключений.
