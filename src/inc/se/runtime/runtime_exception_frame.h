...
#include <se/exception/exception_frame.h>

/**
 * @def se_runtime_exception_frame_save(x)
 * @brief Сохраняет текущую точку возврата для фрейма исключений.
 * @param x Указатель на se_exception_frame_t для сохранения точки возврата.
 * @return Результат выполнения setjmp.
 */
#define se_runtime_exception_frame_save(x) setjmp(se_runtime_exception_frame_push(x)->env)

/**
 * @def se_runtime_exception_frame_load(x)
 * @brief Выполняет возврат к предыдущему фрейму исключений.
 * @param x Значение, возвращаемое при переходе.
 */
#define se_runtime_exception_frame_load(x) longjmp(se_runtime_exception_frame_prev()->env, x)

/**
 * @fn bool se_runtime_exception_frame_is_begin(void)
...
/**
 * @fn bool se_runtime_exception_frame_is_end(void)
...
/**
 * @fn se_exception_frame_t* se_runtime_exception_frame_next(void)
...
/**
 * @fn se_exception_frame_t* se_runtime_exception_frame_push(se_exception_frame_t *except)
...
/**
 * @fn se_exception_frame_t* se_runtime_exception_frame_prev(void)
 * @brief Переходит к предыдущему фрейму исключений в стеке и возвращает его.
 *
 * Уменьшает указатель `se_runtime_exception_frame` на один,
