 ...
 * для экспорта. Он расширяется до `SE_COMPILER_ATTRIBUTE_SYMBOL_EXPORT`, что делает символы
 ...
 * @brief Помечает символ как скрытый.
 *
 * Макрос `SE_ATTRIBUTE_HIDDEN` помечает символы как скрытые и используется для предотвращения
 * их экспорта из библиотеки. Однако, при использовании библиотеки как общей (shared) и
