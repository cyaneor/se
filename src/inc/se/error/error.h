 ...
 *
 * @note Для использования текстового описания ошибки необходимо
 *       определить макрос `SE_LIBRARY_OPTION_ERROR_DESC`.
 */
...
#else
/**
 * @brief Устанавливает код ошибки в объекте se_error_t.
 * @param[in,out] self Указатель на объект se_error_t.
 * @param[in] code Код ошибки для установки.
 */
SE_ATTRIBUTE(SYMBOL)
...
#else
/**
 * @brief Извлекает код ошибки из объекта se_error_t.
 * @param[in] self Указатель на объект se_error_t.
 * @param[out] code Указатель для сохранения кода ошибки (может быть nullptr).
 */
SE_ATTRIBUTE(SYMBOL)
