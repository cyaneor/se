...
#     должна ли библиотека включать поддержку текстовых
#     описаний ошибок в структуре `se_error_t`.
#
#     Установка этой опции в значение ON (по умолчанию) активирует поле `desc`
#     в структуре `se_error_t` и связанные с ним функции, такие как `se_error_set_desc`
#     и `se_error_get_desc`, а также расширенную версию макроса `se_error_set`.
#
#     Это позволяет хранить и получать текстовые описания ошибок, что полезно
#     для отладки и предоставления пользователю более подробной информации об ошибках.
#
# Использование:
#
#     ON: Включает поддержку текстовых описаний ошибок в структуре `se_error_t`.
#         Макрос `se_error_set` будет принимать дополнительный аргумент `desc`
#         для установки описания ошибки, а структура `se_error_t` будет
#         содержать поле `desc` с указателем на строку описания.
#     OFF: Отключает поддержку текстовых описаний ошибок.
#          Поле `desc` в структуре `se_error_t` и связанные функции не будут доступны,
#          а макрос `se_error_set` будет принимать только код ошибки,
#          игнорируя дополнительные аргументы.
#
#          Это уменьшает размер структуры и может быть полезно
#          для экономии памяти в системах с ограниченными ресурсами.
#
# Примечание:
#
#     Включение этой опции улучшает диагностику ошибок,
#     предоставляя дополнительный контекст через текстовые описания.
#
#     Это особенно полезно в сложных приложениях,
#     где понимание причины ошибки критично для разработчиков или пользователей.
#
#     Однако это увеличивает размер структуры `se_error_t` за счёт добавления указателя
#     и может привести к дополнительным накладным расходам на управление памятью
#     для строк описаний.
#
#     Отключение опции может быть предпочтительным в минималистичных
#     или высокопроизводительных системах, где такие описания не требуются.
#
option(SE_LIBRARY_OPTION_ERROR_DESC
        "Включает поддержку текстовых описаний ошибок в структуре se_error_t." OFF)