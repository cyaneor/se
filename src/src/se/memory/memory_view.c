...
#include <se/util/ptr_util.h>

const void *
se_memory_view_get_begin(const se_memory_view_t *self)
{
    se_runtime_check(self, SE_RUNTIME_ERROR_NULL_POINTER, nullptr);
    return self->begin;
}
...
se_memory_view_is_valid(const se_memory_view_t *self)
{
    ...
    return se_ptr_range_valid(begin, end);
}

bool
se_memory_view_contains_pointer(const se_memory_view_t *self, const void *ptr)
{
    se_runtime_check(se_memory_view_is_valid(self), SE_RUNTIME_ERROR_INVALID_MEMORY_RANGE, false);
...
{
    se_runtime_check(element_size, AE_RUNTIME_ERROR_INVALID_ARGUMENT, false);
...
{
    se_runtime_check(alignment_size, AE_RUNTIME_ERROR_INVALID_ARGUMENT, false);

    const void *begin, *end;
    se_memory_view_unpack(self, &begin, &end);
    return se_ptr_pair_aligned(begin, end, alignment_size);
}
...
{
    se_runtime_check(
        se_memory_view_is_valid_offset(self, offset), SE_RUNTIME_ERROR_OUT_OF_RANGE, nullptr);
...
const void *
se_memory_view_at(const se_memory_view_t *self, se_soffset_t offset, bool reversed)
{
